                    ...
                    size_t code_size)
{
    ...
    for (size_t pc = 0; pc < code_size; ++pc)
    {
        ...
        gas_left -= 1;
        if (gas_left < 0)
            return evmc_make_result(EVMC_OUT_OF_GAS, 0, 0, 0, 0, 0, nullptr, 0);

        switch (code[pc])
        {
        default:
            return evmc_make_result(EVMC_UNDEFINED_INSTRUCTION, 0, 0, 0, 0, 0, nullptr, 0);

        case OP_STOP:
            return evmc_make_result(EVMC_SUCCESS, gas_left, 0, 0, 0, 0, nullptr, 0);
        ...
        case OP_MSTORE:
        {
            ...
            if (!memory.store(index, value.bytes, sizeof(value)))
                return evmc_make_result(EVMC_FAILURE, 0, 0, 0, 0, 0, nullptr, 0);
            break;
        }
        ...
        case OP_RETURN:
        {
            ...
            return evmc_make_result(EVMC_SUCCESS, gas_left, 0, 0, 0, 0, output_ptr, output_size);
        }
        ...
        case OP_REVERT:
        {
            ...
            return evmc_make_result(EVMC_REVERT, gas_left, 0, 0, 0, 0, output_ptr, output_size);
        }
        }
    }
