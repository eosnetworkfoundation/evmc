...
{
    {
        ...
        evmc_gas_parameters gas_params;
        const auto result = vm.execute(host, rev, msg, code.data(), code.size(), 0, gas_params);
        ...
        for (int i = 0; i < num_iterations; ++i)
            vm.execute(host, rev, msg, code.data(), code.size(), 0, gas_params);
        ...
        std::ostream& out)
{
    ...
    if (create)
    {
        ...
        evmc_gas_parameters gas_params;
        const auto create_result = vm.execute(host, rev, create_msg, code.data(), code.size(), 0, gas_params);
    ...
    evmc_gas_parameters gas_params;
    const auto result = vm.execute(host, rev, msg, exec_code.data(), exec_code.size(), 0, gas_params);
